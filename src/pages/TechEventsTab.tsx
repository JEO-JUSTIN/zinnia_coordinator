...
export const TechEventsTab: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const isTech = (e: EventItem) => {
        const t = normalize(e.event_type);
        const c = normalize(e.category);
        if (t.includes('NONTECH') || c.includes('NONTECH')) return false;
        return t === 'TECH' || t.includes('TECH') || c.includes('TECH');
      };

      // Filter TECH events visible to coordinator
      const techEvs = data.events.filter(e => {
        if (!isTech(e)) return false;
        // Super Admin and Overall Tech see all tech events
        if (permissions.isSuperAdmin || permissions.isOverallTech) return true;
        // Individual event coordinator can ONLY see their assigned tech event
        if (permissions.assignedTechEventIds.includes(e.id)) return true;
        return false;
      });
      ...
      if (!selectedEventId && techEvs.length > 0) {
        setSelectedEventId(techEvs[0].id);
      }
    } catch (err) {
      console.error('Error loading tech events data:', err);
    }
  };
  ...
  const handleMarkAttendance = async () => {
    ...
    try {
      ...
      const res = await recordAttendance({
        ...
        scanned_by: profile?.full_name || 'Tech Event Coordinator',
        scanned_by_id: profile?.id || 'coord-tech',
        location: currentEvent.venue || 'Lab Complex',
      ...
      console.error('Error recording tech attendance:', err);
      feedback.error();
    ...
    return (
      <div className="max-w-2xl mx-auto bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center text-slate-400">
        ...
        <h3 className="text-base font-bold text-white mb-1">
          {permissions.isSuperAdmin ? 'No Tech Events Found in Database' : 'No Tech Events Assigned'}
        </h3>
        <p className="text-xs">
          {permissions.isSuperAdmin 
            ? 'Super Admin full access is active. No technical events were returned from the events table.'
            : 'You do not have coordinator access to any technical events.'}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-24">
      ...
      {(permissions.isSuperAdmin || permissions.isOverallTech) && events.length > 1 && (
            ...
            <button
              ...
              className={`px-3.5 py-1.5 font-comic tracking-wider uppercase text-xs border-2 transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
                currentEvent?.id === ev.id
                  ? 'bg-[#00F0FF] text-black border-black shadow-comic font-bold -translate-y-0.5'
      ...
      <div className="bg-[#1b1f2d] border-[3.5px] border-black p-5 sm:p-6 shadow-comic-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b-2 border-black">
          <div>
            <div className="inline-block bg-[#00F0FF] text-black font-comic font-bold tracking-wider text-[10px] px-2.5 py-0.5 border border-black uppercase mb-1">
              TECHNICAL EVENT
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-300 font-mono mt-1">
              <span className="flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5 text-[#00F0FF] stroke-[2.5]" />
      ...
      {recordedSuccess ? (
        ...
        <div className="bg-[#1b1f2d] border-[4px] border-black p-6 shadow-comic-xl space-y-4">
          <div className="flex items-start justify-between gap-3 border-b-2 border-black pb-3">
            <div>
              <div className="text-[10px] font-comic tracking-wider uppercase text-[#00F0FF] mb-1">
                ⚡ TECH EVENT CANDIDATE
              </div>
                ...
                {duplicateRecord ? (
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full py-4 px-6 font-comic font-bold tracking-wider text-base sm:text-lg uppercase border-[3.5px] border-black shadow-comic comic-btn flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer bg-[#00F0FF] hover:bg-[#00d4e0] text-black"
                    >
                      {isSubmitting ? (
                        <>
